...
const StackFormModal = ({ show, onClose, stack, onSave }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        const url = isUpdating ? `https://sewedylearn.runasp.net/api/CoreStackWheeler/UpdateCoreW/${stack.id}` : 'https://sewedylearn.runasp.net/api/CoreStackWheeler/AddCoreW';
...
const StacksPageW = () => {
    ...
    const fetchStacks = async () => {
        ...
        try {
            ...
            if (!response.ok) throw new Error('Failed to fetch Wheeler stacks.');
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this stack?')) {
            try {
                const response = await fetch(`https://sewedylearn.runasp.net/api/CoreStackWheeler/DeleteCoreW/${id}`, { method: 'DELETE' });
    ...
    const renderContent = () => {
        ...
        if (stacks.length === 0) return React.createElement('div', { className: 'loading-error-state' }, 'No Wheeler stacks found. Try adding one!');
        ...
        React.createElement('main', null,
            React.createElement('section', { className: 'page-header' },
                React.createElement('div', { className: 'container' },
                    React.createElement('h1', null, 'Wheeler Core Learning Stacks'),
                    React.createElement('p', null, 'Manage the intermediate learning paths for all students.')
                )
            ),
