...
const PageStyles = () => {
    const css = `
        /* Global styles copied from StacksPage */
        ...
        .stacks-page-layout { padding-top: 80px; }
        ...
        .page-header { padding: 60px 0; text-align: center; background-color: var(--background-light); border-bottom: 1px solid var(--border-color); }
        ...
        .stacks-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 30px; }
        .stack-card { background: var(--background-white); border: 1px solid var(--border-color); border-radius: 16px; display: flex; flex-direction: column; box-shadow: 0 4px 20px rgba(0,0,0,0.05); transition: transform 0.3s ease, box-shadow 0.3s ease; }
...
const StacksPageS = () => {
    ...
    const fetchStacks = async () => {
        ...
        try {
            ...
            if (!response.ok) throw new Error('Failed to fetch Senior stacks.');
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this stack?')) {
            try {
                const response = await fetch(`https://sewedylearn.runasp.net/api/CoreStackSenior/DeleteCoreS/${id}`, { method: 'DELETE' });
                if (!response.ok) throw new Error(await response.text() || 'Failed to delete stack.');
    ...
    const renderContent = () => {
        if (isLoading) return React.createElement('div', { className: 'loading-error-state' }, 'Loading Senior Stacks...');
        if (error) return React.createElement('div', { className: 'loading-error-state' }, `Error: ${error}`);
        if (stacks.length === 0) return React.createElement('div', { className: 'loading-error-state' }, 'No Senior stacks found. Try adding one!');
        ...
        React.createElement('main', null,
            React.createElement('section', { className: 'page-header' },
                React.createElement('div', { className: 'container' },
                    React.createElement('h1', null, 'Senior Core Learning Stacks'),
                    React.createElement('p', null, 'Manage the advanced learning paths for all students.')
                )
            ),
